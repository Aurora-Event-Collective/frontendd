...
export default function PollsClient() {
  const [activePolls, setActivePolls] = useState<Poll[]>(pollsData)
      ...
      prevPolls.map((poll) => {
        if (poll.id === pollId && !poll.hasVoted) {
          const updatedOptions = poll.options.map((option) => {
            ...
            return { ...option, percentage: (option.votes / (poll.totalVotes + 1)) * 100 }
          })

          return { ...poll, options: updatedOptions, totalVotes: poll.totalVotes + 1, hasVoted: true }
        }
        return poll
      })
    )
  ...
  return (
    <div className="bg-[#F2EBE3] min-h-screen overflow-x-hidden">
      ...
      <div className="bg-[#214445] text-white py-24 text-center">
        ...
        <p className="text-white/80 max-w-2xl mx-auto">Participate in our community polls to shape the event experience!</p>
      </div>
          ...
          {activePolls.map((poll) => (
            <div key={poll.id} className="bg-white rounded-2xl shadow-xl p-8 border border-[#214445]/10">
              <div className="flex justify-between items-start mb-6">
                <h2 className="text-2xl font-bold text-[#214445]">{poll.question}</h2>
              ...
              <div className="space-y-4 mb-6">
                {poll.options.map((option) => (
                  <div key={option.id} className="relative">
                    <button onClick={() => handleVote(poll.id, option.id)} disabled={poll.hasVoted} className={`w-full text-left p-4 transition-all ${poll.hasVoted ? 'bg-gray-50 border-gray-200 cursor-not-allowed' : 'bg-white border-[#214445]/20 hover:border-[#214445] hover:bg-[#F8F4ED] cursor-pointer'} border-2 rounded-lg`}>
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-semibold text-[#214445]">{option.name}</span>
                        {poll.hasVoted && <span className="text-sm font-bold text-[#214445]">{option.percentage.toFixed(1)}%</span>}
                      </div>

                      <div className="flex justify-between items-center text-sm text-[#5C715E]">
                        <span>{option.votes} votes</span>
                        {poll.hasVoted && <span>{option.percentage.toFixed(1)}%</span>}
                      </div>

                      {poll.hasVoted && <div className="mt-2 w-full bg-gray-200 rounded-full h-2"><div className="bg-[#C89A5B] h-2 rounded-full transition-all duration-500" style={{ width: `${option.percentage}%` }} /></div>}
                    </button>
                  </div>
                ))}
              </div>
